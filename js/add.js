            ...
            for (var i = 1, len = nr_food_items; i < len; i++) {
              ...
              keys.push(item[0]);
                $("<button>" + item[0] + " " + item[1] + " " + item[8] + " " + item[9] + "</button></br>")
                    .val(item[0]).attr('class', 'food-list-btn col-xs-12 btn btn-default' ).attr('id', 'nnn' + i).appendTo('#abcd').click(function () {
...
var calories =0;
            ...
            function button_selected(x) {
                ...
                alert(x.val());
                $("<p>" + x.val() + " " +food_items[x.val()][7] + " kcal" + "</p>").attr('id', x.val()).appendTo('#mealsDiv');
                $("<button>" + "</button>").attr('id', 'btn_' + x.val()).attr('class', 'hfi btn btn-default').appendTo('#' + x.val()).click(function() {remove_food_item($(this)); });
                $("<i>" + "</i>").attr('class', 'glyphicon glyphicon-remove-circle').appendTo('#btn_' + x.val());
